...
-   **Memo Decryption**: Uses `window.hive_keychain.requestVerifyKey(username, encryptedMemo, keyType, callback)` - the EXACT method PeakD uses for decrypting memos. Implements intelligent key type fallback with Shannon entropy detection: tries 'Memo' first (default), then 'Posting', then 'Active' if result is gibberish (entropy > 4.5 or lacks vowels/spaces). This handles messages encrypted with different key types across Hive dApps. 
