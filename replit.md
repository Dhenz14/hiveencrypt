...
-   **Memo Decryption**: Uses multi-key fallback strategy (`Memo` → `Posting` → `Active`) to ensure messages decrypt properly. The app tries each key type and returns the first readable result (determined by entropy and readability checks). This handles messages encrypted by different dApps that may use different key types. 
-   **Double-Encryption Handling**: Automatic detection and recovery for legacy double-encrypted messages (caused by previous bug). If decryption result still starts with '#', automatically attempts second decryption. Recursion limited to max depth of 1 to prevent infinite loops. Properly propagates user cancellation errors.
