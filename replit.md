...
-   **Memo Decryption**: Uses `window.hive_keychain.requestVerifyKey(username, encryptedMemo, 'Memo', callback)` - the EXACT method PeakD uses for decrypting memos. All Hive encrypted memos use the memo key type as per the standard. Users only sign with their memo key for decryption, ensuring minimal key exposure. 
