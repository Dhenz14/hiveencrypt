...
import { isKeychainAvailable } from '@/lib/keychainDetection';
import { requestHandshake } from '@/lib/hive';
import { useToast } from '@/hooks/use-toast';

export default function Login() {
  const [, setLocation] = useLocation();
  const { login, platform } = useAuth();
  const { toast } = useToast();
  const [username, setUsername] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [keychainStatus, setKeychainStatus] = useState<'checking' | 'ready' | 'missing'>('checking');

  useEffect(() => {
    const checkKeychain = async () => {
      const available = isKeychainAvailable();
      if (available) {
        // Try handshake to verify Keychain is responsive
        try {
          await requestHandshake();
          setKeychainStatus('ready');
        } catch (error) {
          // Handshake may fail but extension could still be installed
          setKeychainStatus('ready');
        }
      } else {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!isKeychainAvailable()) {
      toast({
        ...
        description: 'Please ensure Hive Keychain is available',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (errorMessage.includes('install') || errorMessage.includes('Keychain')) {
        toast({
          title: 'Hive Keychain Not Found',
          description: 'Please ensure Hive Keychain extension is installed or you are using Keychain Mobile browser',
          variant: 'destructive',
        });
  ...
  const isKeychainMobile = platform === 'keychain-mobile-browser';
        ...
        <Card className="shadow-lg">
          <CardHeader className="space-y-1">
            ...
            <CardDescription className="text-body">
              {isKeychainMobile ? 
                'Authenticate with Hive Keychain Mobile' : 
          ...
          <CardContent className="space-y-4">
            ...
            {keychainStatus === 'missing' && (
              <Alert variant="destructive" className="border-destructive/50">
                ...
                <AlertDescription className="text-caption">
                  Hive Keychain not detected. Please install it from{' '}
                  <a 
            ...
            {keychainStatus === 'ready' && (
              <Alert className="bg-primary/5 border-primary/20">
                ...
                <AlertDescription className="text-caption text-foreground">
                  {isKeychainMobile ? 'Keychain Mobile ready' : 'Hive Keychain detected and ready'}
                </AlertDescription>
              </Alert>
            )}
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <Button
                ...
                disabled={isLoading || keychainStatus === 'missing'}
